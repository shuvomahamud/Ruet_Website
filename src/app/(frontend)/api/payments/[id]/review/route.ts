...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    try {
      ...
      payload.logger.error({ err: error, msg: 'Payment review notification could not queue.' })
    }
    const target = result.payment.orderTypeSnapshot === 'event' ? 'event registration' : 'membership'
    return Response.json({
      message: result.idempotent
        ? 'This payment decision was already recorded.'
        : input.data.decision === 'approve'
          ? `The payment was approved and the ${target} was activated.`
          : 'The payment was rejected and can be resubmitted as a new attempt.',
    })
  } catch (error) {
    const status =
    ...
    return Response.json(
      {
        message: error instanceof AppError ? error.message : 'The payment review could not be completed.',
      },
