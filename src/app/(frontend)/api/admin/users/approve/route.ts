...
import { approveUsers } from '@/services/admin-bulk-actions'
...
export async function POST(request: Request) {
  ...
  const result = await approveUsers(await createLocalReq({ user }, payload), input.data.ids)
  return Response.json({ ...result, message: `${result.succeeded.length} account(s) approved; ${result.failed.length} failed.` })
}
